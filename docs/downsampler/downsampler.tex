...
Another note is that my implementation uses the filtered/scaled ``$\sigma_{orig}$'' to compute $\bar{\varphi}$, thus, the true error between the sampled data will likely be slightly higher.
...
\begin{itemize}
    ...
    \item $\eta = 0.99$
    \item $n_{cyc} = 20$
    \item $\epsilon = 0.001$ or $\epsilon_g = 0.009$
    \item $w = 5$
    \item $\alpha = 11$
    \item $p = 1$
    \item $n_{elastic} = 7$
...
A value of of $\epsilon_g = 0.009$ is recommended if $\bar{\varphi}$ is computed based on the filtered/normalized stress-strain data so that the final overall error is around 1~\%.
